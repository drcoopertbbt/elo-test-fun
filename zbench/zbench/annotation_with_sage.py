"""
ZBench annotation pipeline with SageMath high-precision engine
Simplified version that uses the new main.py interface
"""
...
from zbench.sage_engine import calculate_elos_with_sage_backend

class HighPrecisionAnnotator:
    """ZBench annotator with SageMath high-precision Elo calculation"""
    ...
    def step4_compose_annotated_dataset(self) -> None:
        """Compose annotated dataset using SageMath high-precision calculation"""
        print("Step 4: Composing annotated dataset with SageMath high-precision engine")
        ...
        # Process each query
        for data in tqdm(dataset.data, desc="Calculating high-precision Elos"):
            documents = data.documents
            ...
            for scored_pair in scores[data.query.id].scored_pairs:
                ...
                score = 0.0
                # Use 2-provider ensemble (OpenAI + Anthropic, skip Gemini)
                if scored_pair.openai_score.score > 0:
                    score += 1/2
                if scored_pair.anthropic_score.score > 0:
                    score += 1/2
                # Gemini is dummy score (always 0.0), so we skip it
                w[j,i] += score
                w[i,j] += 1 - score
            
            # Use SageMath high-precision calculation
            document_ids = [doc.id for doc in documents]
            elos, status = calculate_elos_with_sage_backend(w, document_ids)
        ...
        print(f"Created {len(annotated_dataset.data)} annotated lines with high-precision Elos")
        print(f"Saved annotated dataset to {self.annotated_dataset_path}")
    
    async def annotate_with_precision(self) -> None:
        """Run the complete high-precision annotation pipeline"""
    
        try:
            print("\n🔬 Starting ZBench with SageMath High-Precision Engine")
            print("=" * 60)
            ...
            # Step 4: Calculate high-precision Elos
            self.step4_compose_annotated_dataset()
            
            print(f"\n🎉 High-precision annotation complete!")
            print(f"Output: {self.annotated_dataset_path}")
...
async def main():
    parser = argparse.ArgumentParser(description="ZBench annotation with SageMath high-precision Elo calculation")
    parser.add_argument("dataset_path", help="Path to the dataset jsonl file")
    parser.add_argument("annotated_dataset_path", help="Path to the output annotated dataset")
    ...
    processor = HighPrecisionAnnotator(
    ...
    await processor.annotate_with_precision()
